...
//  Reference : https://github.com/teodorpatras/EasyTipView

import UIKit

// MARK:- Delegate

public protocol DNATooltipDelegate : class {
    func toolkitDidDismiss(_ tipView : DNATooltip)
}
...
public extension DNATooltip {
    
    // MARK:- Class methods -
    
    /**
     Presents an DNATooltip pointing to a particular UIBarItem instance within the specified superview
     
     - parameter animated:    Pass true to animate the presentation.
     - parameter item:        The UIBarButtonItem or UITabBarItem instance which the DNATooltip will be pointing to.
     - parameter superview:   A view which is part of the UIBarButtonItem instances superview hierarchy. Ignore this parameter in order to display the DNATooltip within the main window.
     - parameter text:        The text to be displayed.
     - parameter preferences: The preferences which will configure the DNATooltip.
     - parameter delegate:    The delegate.
     */
    class func show(animated: Bool = true, forItem item: UIBarItem, withinSuperview superview: UIView? = nil, text: String, preferences: Preferences = DNATooltip.globalPreferences, delegate: DNATooltipDelegate? = nil){
        
        let view = UIView(frame: item.accessibilityFrame)
        show(animated: animated, forView: view, withinSuperview: superview, text: text, preferences: preferences, delegate: delegate)
        
    }
    
    /**
     Presents an DNATooltip pointing to a particular UIView instance within the specified superview
     
     - parameter animated:    Pass true to animate the presentation.
     - parameter view:        The UIView instance which the DNATooltip will be pointing to.
     - parameter superview:   A view which is part of the UIView instances superview hierarchy. Ignore this parameter in order to display the DNATooltip within the main window.
     - parameter text:        The text to be displayed.
     - parameter preferences: The preferences which will configure the DNATooltip.
     - parameter delegate:    The delegate.
     */
    class func show(animated: Bool = true, forView view: UIView, withinSuperview superview: UIView? = nil, text:  String, preferences: Preferences = DNATooltip.globalPreferences, delegate: DNATooltipDelegate? = nil){
        
        let ev = DNATooltip(text: text, preferences: preferences, delegate: delegate)
    ...
    /**
     ...
     - parameter delegate:    The delegate.
     */
    class func show(animated: Bool = true, forItem item: UIBarItem, withinSuperview superview: UIView? = nil, contentView: UIView, preferences: Preferences = DNATooltip.globalPreferences, delegate: DNATooltipDelegate? = nil){
        
        if let view = item.value(forKeyPath: "view") as? UIView {
    ...
    /**
     ...
     - parameter delegate:    The delegate.
     */
    class func show(animated: Bool = true, forView view: UIView, withinSuperview superview: UIView? = nil, contentView: UIView, preferences: Preferences = DNATooltip.globalPreferences, delegate: DNATooltipDelegate? = nil){
        
        let ev = DNATooltip(contentView: contentView, preferences: preferences, delegate: delegate)
        ev.show(animated: animated, forView: view, withinSuperview: superview)
    }
    ...
    func show(animated: Bool = true, forView view: UIView, withinSuperview superview: UIView? = nil) {
        ...
        tapGestureRecogniser = UITapGestureRecognizer(target: self, action: #selector(handleSuperViewTap))
        
        superview.addGestureRecognizer(tapGestureRecogniser!)
    ...
    @objc func handleSuperViewTap() {
        self.dismiss()
    }
    
    /**
     Dismisses the DNATooltip
     
     - parameter completion: Completion block to be executed after the DNATooltip is dismissed.
     */
    func dismiss(withCompletion completion: (() -> ())? = nil){
        if ((superview?.gestureRecognizers) != nil) {
        ...
        UIView.animate(withDuration: preferences.animating.dismissDuration, delay: 0, usingSpringWithDamping: damping, initialSpringVelocity: velocity, options: [.curveEaseInOut], animations: {
            ...
            completion?()
            self.delegate?.toolkitDidDismiss(self)
...
extension DNATooltip: UIGestureRecognizerDelegate {
...
// MARK: - DNATooltip class implementation
open class DNATooltip: UIView {
    ...
    public struct Preferences {
        ...
        public struct Drawing {
            ...
            public var backgroundColor     = Color.appPrimary
            public var arrowPosition       = ArrowPosition.any
            public var textAlignment       = NSTextAlignment.center
            public var borderWidth         = CGFloat(0)
            public var borderColor         = UIColor.clear
            public var font                = DNAFont.regular.of(size: 14)
        ...
        public struct Animating {
            ...
            public var showDuration         = 0.4
            public var dismissDuration      = 0.4
    ...
    //To dismiss tooltip on tap anywhere in superview
    public var tapGestureRecogniser : UIGestureRecognizer?
    ...
    fileprivate weak var delegate: DNATooltipDelegate?
    ...
    fileprivate lazy var contentSize: CGSize = {
        ...
        case .text(let text):
            ...
            var attributes = [NSAttributedString.Key.font : self.preferences.drawing.font, NSAttributedString.Key.paragraphStyle: paraStyle]
    ...
    public convenience init (text: String, preferences: Preferences = DNATooltip.globalPreferences, delegate: DNATooltipDelegate? = nil) {
        self.init(content: .text(text), preferences: preferences, delegate: delegate)
    }
    
    public convenience init (contentView: UIView, preferences: Preferences = DNATooltip.globalPreferences, delegate: DNATooltipDelegate? = nil) {
        self.init(content: .view(contentView), preferences: preferences, delegate: delegate)
    }
    
    private init (content: Content, preferences: Preferences = DNATooltip.globalPreferences, delegate: DNATooltipDelegate? = nil) {
    ...
    fileprivate func computeFrame(arrowPosition position: ArrowPosition, refViewFrame: CGRect, superviewFrame: CGRect) -> CGRect {
        ...
        switch position {
        case .top, .any:
            xOrigin = refViewFrame.midX - tipViewSize.width / 2
            yOrigin = refViewFrame.minY + refViewFrame.height
        case .bottom:
            xOrigin = refViewFrame.midY - tipViewSize.width / 2
            yOrigin = refViewFrame.minY - tipViewSize.height
        case .right:
            xOrigin = refViewFrame.minX - tipViewSize.width
            yOrigin = refViewFrame.midY - tipViewSize.height / 2
        case .left:
        ...
        if !isFrameValid(frame, forRefViewFrame: refViewFrame, withinSuperviewFrame: superviewFrame) {
            for value in ArrowPosition.allValues where value != position {
                ...
                if isFrameValid(newFrame, forRefViewFrame: refViewFrame, withinSuperviewFrame: superviewFrame) {
                    ...
                    if position != .any {
                        print("[DNATooltip - Info] The arrow position you chose <\(position)> could not be applied. Instead, position <\(value)> has been applied! Please specify position <\(ArrowPosition.any)> if you want DNATooltip to choose a position for you.")
                    }
